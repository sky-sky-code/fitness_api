...
@router.put('/gymroom/{uid:uuid}', response_model=GymRoom_OfficePK_Pydantic)
async def update_gymroom(uid: uuid.UUID, gymroom: GymRoom_OfficePK_Pydantic):
    get_gym = await models.GymRoom.get(uid=uid)
    update_gym = await get_gym.update_from_dict(gymroom.dict())
    await update_gym.save()
    return await GymRoom_OfficePK_Pydantic.from_tortoise_orm(update_gym)


@router.delete('/gymroom/{uid:uuid}', response_model=GymRoom_OfficePK_Pydantic)
